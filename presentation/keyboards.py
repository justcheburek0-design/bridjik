...
class KeyboardBuilder:
    ...
    def freeze_keyboard(self, user_id: int, freeze_options: tuple, hot: bool = True) -> types.InlineKeyboardMarkup:
        """Build freeze/unfreeze keyboard."""
        buttons = [
            types.InlineKeyboardButton(text=get_hour_string(hours), callback_data=f"freeze:{user_id}:{hours}")
            for hours in freeze_options
        ]
        rows = [buttons[i:i + 2] for i in range(0, len(buttons), 2)]
        
        if hot:
            rows.append([types.InlineKeyboardButton(text="🔥 Разморозка 🔥", callback_data=f"unfreeze:{user_id}")])
