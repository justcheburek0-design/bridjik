...
class RAGService:
    ...
    def __init__(self, config: Config, mc_api: MinecraftAPI, mb_api: MineBridgeAPI):
        self.config = config
        self.mc_api = mc_api
        self.mb_api = mb_api
        ...
        async with self._rag_lock:
            ...
            if not need_rebuild:
                for p in kb_files:
                    m = p.stat().st_mtime
                    if not any(
                        c["file"] == str(p) and abs(c.get("mtime", 0.0) - m) < 1e-6
            ...
            for p in kb_files:
                ...
                parts = self._split_chunks(
                    clean, self.config.RAG_CHUNK_SIZE, self.config.RAG_CHUNK_OVERLAP
                )
                m = p.stat().st_mtime
                for i, ch in enumerate(parts):
                    cid = f"{self._hash_str(str(p))}:{i}"
                    all_chunks.append(
                        {"id": cid, "file": str(p), "text": ch, "mtime": m, "priority": pr}
                    )
                    all_texts.append(ch)
            ...
            if vecs:
                ...
                self._rag_loaded = True
                logger.info(
                    "RAG: built %d chunks from %d files", len(self._rag_chunks), len(kb_files)
                )
            else:
    ...
    async def search(self, query: str) -> List[Tuple[dict, float]]:
        ...
        weights = np.array(
            [max(0.1, (float((c.get("priority", 5) or 5)) / 5.0)) for c in self._rag_chunks],
            dtype="float32",
        )
        adj = sims * weights
    ...
    async def build_structured_context(self, prompt: str, user_id: Optional[int] = None) -> dict:
        ...
        if psevdo:
            context["user_info"] = {"psevdo": psevdo, "telegram_id": user_id}
